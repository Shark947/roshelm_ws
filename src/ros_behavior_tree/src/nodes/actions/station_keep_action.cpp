...
StationKeepAction::StationKeepAction(const std::string &name,
...
BT::PortsList StationKeepAction::providedPorts()
{
  return {BT::InputPort<std::string>("params")};
}

BT::NodeStatus StationKeepAction::onStart()
{
  return runBehavior();
}

BT::NodeStatus StationKeepAction::onRunning()
{
  return runBehavior();
}
...
bool StationKeepAction::ensureBehavior()
{
  ...
  if (!behavior_)
  {
    behavior_.reset(new StationKeepBehavior(helm_interface_->domain()));
  }
  return true;
}

bool StationKeepAction::configureBehavior(const std::string &params)
{
  if (!behavior_)
    return false;
  ...
  for (const auto &entry : parsed)
  {
    if (entry.first == "name")
    {
      ...
      {
        ROS_WARN_STREAM("[ros_behavior_tree] Failed to set StationKeep name: "
                        << entry.second);
      }
      continue;
    }
    ...
    {
      ROS_WARN_STREAM("[ros_behavior_tree] Failed to set StationKeep param: "
...
BT::NodeStatus StationKeepAction::runBehavior()
{
  if (!helm_interface_)
