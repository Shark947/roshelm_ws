...
- `config/alpha.xml` 通过 `DeployTriggered`、`ReturnTriggered`、
  ...
  （依据航点引擎的 cycle 计数）发布 `RETURN=true`，返航完成时发布
  `RETURN=false`、`DEPLOY=false` 与 `MISSION=complete`。`endflag` 也支持发布任意自定义变量，默认映射为
...
部署触发的场景。
`default_mode_on_deploy` 为 `true` 时，当收到 `DEPLOY=true` 会自动写入
`MODE=default_mode_value`（默认 `LIFT`），保持与 `ros_helm` 方案一致，
无需额外手动发布 `MODE`。
