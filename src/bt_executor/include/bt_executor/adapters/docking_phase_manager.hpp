...
struct DockingPhaseManagerConfig
{
  double nav_server_period{0.1};
  ...
  double distance_bias{0.3};
  double angle_bias_deg{0.5};
  double fall_depth{0.0};
  double camera_view_angle_deg{25.0};
  int max_try_minute_num{1};
  int constant_depth_minute_num{5};
  double depth_ground_bias{0.0};
  double depth_camera_bias{0.0};
  double dock_panel{0.0};
  double last_phase_float_time_ms{500.0};
  int transmit_duration_sec{30};
  int docking_max_try{3};
  double optical_camera_delta_l{0.0};

  std::vector<double> align_depth_offsets;

  double dradius{0.0};
  double dinheading_deg{0.0};
  double radius1th{0.0};

  bool auto_enter_closetodocking{false};
  double auto_enter_duration_sec{2.0};
...
public:
  ...
  bool initialize(const DockingPhaseManagerConfig &config);

  void setOpticalMeasurement(const OpticalMeasurementData &measurement);
  void setDesiredSpeed(double speed);
  void setDockDepth(double depth);

  DockingPhaseResult tick(const NavSnapshot &nav, const MissionSnapshot &mission);
...
private:
  ...
  void handleAutoEnterCloseToDocking(const MissionSnapshot &mission, const NavSnapshot &nav,
                                     DockingPhaseResult &result);
  void handleDocking(const NavSnapshot &nav, const MissionSnapshot &mission,
                     DockingPhaseResult &result);

  void updateOutputs(const NavSnapshot &nav, DockingPhaseResult &result);
  ...
  OpticalMeasurementData optical_measurement_{};
