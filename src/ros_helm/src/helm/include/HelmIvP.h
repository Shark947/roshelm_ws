...
class HelmIvP : public HelmApp
{
public:
  HelmIvP();
  virtual ~HelmIvP();
  
  bool OnNewMail(std::list<HelmMsg> &NewMail);
  bool Iterate();
  ...
  const IvPDomain &getIvPDomain() const { return m_ivp_domain; }
  const HelmReport &getHelmReport() const { return m_helm_report; }
 ...
 protected:
  ...
  bool updateInfoBuffer(HelmMsg &Msg);
  void postHelmStatus();
  void postCharStatus();
  ...
  std::vector<std::string> collectAdditionalRegisterVariables() const override;
  ...
  void        seedInfoBuffer(const std::map<std::string, double> &dvalues,
