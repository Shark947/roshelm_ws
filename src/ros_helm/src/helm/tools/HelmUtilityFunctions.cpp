...
#define ENABLE_WIN32_HPHelmTime 0
...
double gdfHelmTimeWarp = 1.0;
double gdfMOOSSkew =0.0;
static bool gHelmPlayback = false;

void SetHelmPlayBack(bool bPlayBack) { gHelmPlayback = bPlayBack; }
bool IsHelmPlayBack() { return gHelmPlayback; }
...
namespace Helm
{
	...
	std::string TimeToDate(double dfTime, bool bDate, bool bTime)
    {
        ...
        time_t nowtime = TimeVal.tv_sec;
        struct tm* nowtm = localtime(&nowtime);

        // 用 string 接管输出，避免 snprintf 警告和溢出
        if (!nowtm) {
            // dfTime < 0 的情况
            return std::to_string(dfTime);
        }
        ...
        // 毫秒部分（C++安全拼接，不再引发警告）
        int ms = TimeVal.tv_usec / 1000;
        std::string time_with_ms = std::string(stime) + "." + 
                                (ms < 100 ? (ms < 10 ? "00" : "0") : "") +
                                std::to_string(ms);
...
void    SetMOOSSkew(double dfSkew)
{
    ...
    //a call to HPHelmTime() sets up high performance clocks
    HPHelmTime();
}
...
double MOOSLocalTime(bool bApplyTimeWarping)
{
	...
	{	
		...
		{			
			...
			return bApplyTimeWarping ? dfMOOSStart*gdfHelmTimeWarp :dfMOOSStart;
		...
		else
		{
            ...
            return bApplyTimeWarping ? T*gdfHelmTimeWarp :T;
...
double HPHelmTime(bool bApplyTimeWarping)
{
	return HelmTime(bApplyTimeWarping);
}

double HelmTime(bool bApplyTimeWarping)
{
    return MOOSLocalTime(bApplyTimeWarping)+gdfMOOSSkew;
}

double GetHelmTimeWarp()
{
    return gdfHelmTimeWarp;
}

bool SetHelmTimeWarp(double dfWarp)
{
    if(dfWarp>0 && dfWarp<=MAX_TIME_WARP)
    {
        gdfHelmTimeWarp = dfWarp;
        return true;
    }
...
{
#ifdef _WIN32
	//nMS = int(double(nMS)/gdfHelmTimeWarp);
	::Sleep(nMS);
#else

    double dfMilliSeconds = nMS;
    if(bApplyTimeWarping)
        dfMilliSeconds/=gdfHelmTimeWarp;
