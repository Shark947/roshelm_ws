...
// HelmMsg.cpp: implementation of the HelmMsg class.
//
//////////////////////////////////////////////////////////////////////
#ifdef _WIN32
...
#include "HelmException.h"
#include "HelmPlaybackStatus.h"
#include "HelmMsg.h"
...
HelmMsg::HelmMsg()
    : m_cMsgType(HELM_NULL_MSG),
      m_cDataType(HELM_DOUBLE),
      m_nID(-1),
      m_dfTime(-1),
      m_dfVal(-1),
      m_dfVal2(-1),
...
HelmMsg::HelmMsg(char cMsgType, const std::string &sKey, double dfVal,
                 double dfTime, const std::string &sSrcAux)
    : m_cMsgType(cMsgType),
      m_cDataType(HELM_DOUBLE),
      m_sKey(sKey),
      m_nID(-1),
      m_dfTime((dfTime == -1) ?  HelmTime() : dfTime),
      m_dfVal(dfVal),
      m_dfVal2(-1),
      m_sSrcAux(sSrcAux) {}

HelmMsg::HelmMsg(char cMsgType, const std::string &sKey,
                 const std::string &sVal, double dfTime,
                 const std::string &sSrcAux)
    : m_cMsgType(cMsgType),
      m_cDataType(HELM_STRING),
      m_sKey(sKey),
      m_nID(-1),
      m_dfTime((dfTime == -1) ?  HelmTime() : dfTime ),
      m_dfVal(-1),
...
HelmMsg::HelmMsg(char cMsgType, const std::string &sKey,
                 ...
                 const std::string &sSrcAux)
    : m_cMsgType(cMsgType),
      m_cDataType(HELM_BINARY_STRING),
      m_sKey(sKey),
      m_nID(-1),
      m_dfTime((dfTime == -1) ?  HelmTime() : dfTime),
      m_dfVal(-1),
...
bool HelmMsg::operator == (const HelmMsg & M) const
{
...
void  HelmMsg::operator << (double & dfVal)
{
    ...
    {
        throw CMOOSException("HelmMsg::operator << Out Of Space");
    }
}
...
void  HelmMsg::operator << (string &   sVal)
{
/*
...
void  HelmMsg::operator << (int & nVal)
{
...
void  HelmMsg::operator << (char &  cVal)
{
...
unsigned int HelmMsg::GetSizeInBytesWhenSerialised() const
{
...
int HelmMsg::Serialize(unsigned char *pBuffer, int nLen, bool bToStream)
{
    ...
    if(bToStream)
    {
        ...
        {
            MOOSTrace("exception : HelmMsg::Serialize failed: %s\n ",e.m_sReason);
...
int HelmMsg::GetLength()
{
    return m_nLength;
}

bool HelmMsg::CanSerialiseN(int N)
{
    return      (m_pSerializeBuffer-m_pSerializeBufferStart)+N<=m_nSerializeBufferLen;
}

bool HelmMsg::IsType(char cType) const
{
    return m_cMsgType==cType;
}

char HelmMsg::GetType() const
{
	return m_cMsgType;
}
...
bool HelmMsg::IsYoungerThan(double dfAge) const
{
    return m_dfTime>=dfAge;
}

/**
*A method to check the timestamping of a HelmMsg.
...
*HelmMsg's time stamp (<code>m_dfTime</code>) to HELM_SKEW_TOLERANCE.
*@return true if a HelmMsg's time stamp is either HELM_SKEW_TOLERANCE seconds ahead or
*behind the MOOSDB clock.  Will also pass you the <code>pdfSkew</code>, or amount of
*time difference between the MOOSDB and HelmMsg timestamp if desired. 
*/
bool HelmMsg::IsSkewed(double dfTimeNow, double * pdfSkew)
{
...
//    }
    ...
    return (dfSkew > HELM_SKEW_TOLERANCE) ? true : false;
...
string HelmMsg::GetAsString(int nFieldWidth/*=12*/, int nNumDP/*=5*/)
{
    ostringstream os;
    ...
    if(GetTime()!=-1)
    {        
        if(IsDataType(HELM_DOUBLE))
        {
			os.setf(ios::fixed);

            os<<setw(nFieldWidth)<<setprecision(nNumDP)<<m_dfVal;//<<ends;
        }
		else if(IsDataType(HELM_BINARY_STRING))
		{
...
unsigned int HelmMsg::GetBinaryDataSize()
{
	if(!IsBinary())
		return 0;
	else
...
bool HelmMsg::GetBinaryData(std::vector<unsigned char > &v)
{
	if(!IsBinary())
		return false;
...
std::vector<unsigned char >  HelmMsg::GetBinaryDataAsVector()
{
...
unsigned char * HelmMsg::GetBinaryData()
{
	if(!IsBinary())
		return NULL;
	else
...
bool  HelmMsg::IsName(const std::string & sName)
{
	return MOOSStrCmp(m_sKey, sName);
}


bool HelmMsg::IsDataType(char cDataType) const
{
