    ...
    * `axis` (*type:* `numpy.array`): Axis of rotation of the thruster.
    ...
    def __init__(self, index, topic, pos, orientation, axis=DEFAULT_AXIS):
        ...
        if pos is not None and orientation is not None:
            ...
            rospy.loginfo('axis' )
            rospy.loginfo(axis)
            rospy.loginfo('quaternion_matrix(orientation):')
            rospy.loginfo(transformations.quaternion_matrix(orientation))
            rospy.loginfo('dot:')
            rospy.loginfo(transformations.quaternion_matrix(orientation).dot(
                axis.transpose()))
            rospy.loginfo('thrust_body: ' )
            rospy.loginfo(thrust_body )
            rospy.loginfo('torque_body: ' )
            rospy.loginfo(torque_body )
            rospy.loginfo('self._force_dist: ' )
            rospy.loginfo(self._force_dist )
        self._command = 0
        self._thrust = 0
