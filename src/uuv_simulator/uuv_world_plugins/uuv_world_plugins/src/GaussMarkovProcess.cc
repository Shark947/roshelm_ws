...
double GaussMarkovProcess::Update(double _time)
{
  ...
  std::default_random_engine generator_gm = std::default_random_engine(rd());
  // 初始化正态分布，均值为0，标准差为1（后续根据 noiseAmp 调整）
 // std::normal_distribution<double> distribution_gm = std::normal_distribution<double>(0.0, 1.0);
  //定义正态分布的标准差为 noiseAmp
std::normal_distribution<double> distribution_gm = std::normal_distribution<double>(0.0, this->noiseAmp);
  // 使用正态分布生成随机数
  double random = distribution_gm(generator_gm);

  this->var = (1 - step * this->mu) * this->var + random;
